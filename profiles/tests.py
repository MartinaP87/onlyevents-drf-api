...
class PreferenceListViewTests(APITestCase):
    ...
    def test_can_list_preferences(self):
        ...
        Preference.objects.create(profile=profile, genre=genre)
        response = self.client.get('/profiles/preferences/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_logged_in_user_can_create_a_preference(self):
        ...
        response = self.client.post(
            '/profiles/preferences/', {'genre': genre.id})
...
class PreferenceDetailViewTests(APITestCase):
    ...
    def test_user_can_update_own_preference(self):
        ...
        self.client.login(username='marla', password='pass')
        response = self.client.put(
            '/profiles/preferences/1/', {'genre': genre2.id})
        preference = Preference.objects.filter(pk=1).first()
    ...
    def test_user_cant_update_another_users_preference(self):
        ...
        self.client.login(username='marla', password='pass')
        response = self.client.put(
            '/profiles/preferences/2/', {'genre': genre1.id})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_user_can_delete_own_preference(self):
        preference1 = Preference.objects.get(pk=1)
        self.client.login(username='marla', password='pass')
        response = self.client.delete('/profiles/preferences/1/')
        response2 = self.client.get('/profiles/preferences/1/')
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(response2.status_code, status.HTTP_404_NOT_FOUND)

    def test_user_cant_delete_someone_else_preference(self):
        preference2 = Preference.objects.get(pk=2)
        self.client.login(username='marla', password='pass')
        response = self.client.delete('/profiles/preferences/2/')
        response2 = self.client.get('/profiles/preferences/2/')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
